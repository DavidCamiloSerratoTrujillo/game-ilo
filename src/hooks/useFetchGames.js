...
export const useFetchGames = (orderby) => {
    ...
    useEffect(()=>{
        getGames(orderby).then(gm =>{//El get Gif es un helper que basicamente es una funcion que trae las imagenes de un json
            setState({
                data: gm,
                loading: false
    ...
    },[orderby])//Usamos el useefect para que se renderice solo cuando se modifica la categoria por eso se encierra en llaves cuadradas
